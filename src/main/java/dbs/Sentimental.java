	 ...
	 public static void cross_reference()
	 {
	    ...
	    {	
    		if(!input_path.equals(input_Dir))
    			...
    			filterdatei = sparkSession.sql("Select full_text from zu_pruefen_view , filter_liste WHERE zu_pruefen_view.text LIKE ('%' || ' ' || filter_liste.term || ' ' || '%') ");  //pruefen auf enthalten der Filterliste
